...
describe('POST /readings (createReading)', () => {
  ...
  describe('Contract definition', () => {
    ...
    it('should have ReadingCreateRequest source enum', () => {
      const spec = loadContract();
      const schemas = (spec.components as Record<string, unknown>).schemas as Record<string, unknown>;
      const props = (schemas.ReadingCreateRequest as Record<string, unknown>).properties as Record<string, unknown>;
      expect((props.source as Record<string, unknown>).enum).toEqual(['manual', 'import', 'automatic']);
    });
  });
  ...
  describe('Request schema validation', () => {
    ...
    it('should validate a valid ReadingCreateRequest', () => {
      const spec = loadContract();
      const schemas = (spec.components as Record<string, unknown>).schemas as Record<string, unknown>;
      const result = validateResponseBody(schemas.ReadingCreateRequest as Record<string, unknown>, validBody);
    ...
    it('should accept optional rawPayload', () => {
      ...
      const result = validateResponseBody(schemas.ReadingCreateRequest as Record<string, unknown>, body);
      expect(result.valid).toBe(true);
    });
    ...
    it('should reject invalid source enum', () => {
      const spec = loadContract();
      const schemas = (spec.components as Record<string, unknown>).schemas as Record<string, unknown>;
      const result = validateResponseBody(schemas.ReadingCreateRequest as Record<string, unknown>, { ...validBody, source: 'invalid' });
      expect(result.valid).toBe(false);
    });
  });
  ...
  describe('HTTP endpoint (TDD — expected to fail before implementation)', () => {
    ...
    it('should return a valid status code for createReading', async () => {
      const res = await request.post('/api/v1/readings').send(validBody);
