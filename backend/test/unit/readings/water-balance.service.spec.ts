...
describe('WaterBalanceService', () => {
  it('should calculate correctly', async () => {
    ...
    m.meter.findMany.mockResolvedValueOnce([{ id: 'c1', serialNumber: 'WC-1' }, { id: 'c2', serialNumber: 'WC-2' }]);
  ...
  it('should throw on missing project', async () => {
    ...
    const svc = await createService(m);
    await expect(svc.getWaterBalance('bad', new Date(), new Date())).rejects.toThrow('Project not found');
  });
  ...
  it('should throw on no main meters', async () => {
    ...
    const svc = await createService(m);
    await expect(svc.getWaterBalance('p1', new Date(), new Date())).rejects.toThrow('No water main meters');
  });
